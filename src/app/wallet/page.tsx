...
function WalletContent() {
  ...
  const handleAddFunds = async (e: React.FormEvent) => {
    ...
    if (!amount || isNaN(numAmount) || numAmount < minAllowed) {
      toast.error(
        `Minimum transaction amount is ${currencySymbol}${minAllowed.toFixed(isUSD ? 2 : 0)}.`
      );
      return;
    }
            ...
            >
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <p className="text-xs font-bold text-[#6B5E55] uppercase tracking-wider">
                    Available Balance
                  </p>
                </div>
                ...
                <p className="text-xs text-[#6B5E55] mb-4">
                  Securely store funds for instant consultations
                </p>
                ...
                <div className="pt-3 border-t border-[#E5D9C8] flex items-start gap-2.5 text-xs text-[#713B32] bg-[#EDE4D5]/60 p-3 rounded-2xl">
                  ...
                  <span>
                    Note: A minimum wallet balance for <strong>5 minutes</strong> of consultation is
                    required to connect with an astrologer.
                  </span>
                </div>
              </div>
            </motion.div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-xs text-[#6B5E55]">
                      Min: {currencySymbol}
                      {minAllowed.toFixed(isUSD ? 2 : 0)}
                    </span>
                  </div>
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6B5E55] font-bold">
                      {currencySymbol}
                    </span>
                    <input
                      type="number"
                  ...
                  {isTooLow && (
                    ...
                    >
                      <span>
                        ⚠️ Minimum transaction amount is {currencySymbol}
                        {minAllowed.toFixed(isUSD ? 2 : 0)}. Payments below this amount are not
                        accepted.
                      </span>
                    </motion.p>
                  )}
          ...
          <div className="lg:col-span-2">
            ...
            >
              <div className="flex items-center justify-between mb-6 border-b border-[#E5D9C8] pb-4">
                ...
                <span className="text-xs font-bold text-[#6B5E55] bg-[#EDE4D5] px-3 py-1 rounded-full">
              ...
              {loadingTx ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <p className="text-xs text-[#6B5E55]">
                    Your wallet history will appear here once you add funds.
                  </p>
                </div>
              ) : (
                <div className="space-y-3 overflow-y-auto pr-2 max-h-[600px] custom-scrollbar">
                  {transactions.map((tx) => (
                    <div
                      key={tx.id}
                      className="flex items-center justify-between p-4 rounded-2xl bg-[#F8F3EA] hover:bg-[#EDE4D5]/60 transition-colors border border-[#E5D9C8]"
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center ${tx.status === 'failed' ? 'bg-red-100 text-red-600' : tx.type === 'credit' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}
                        >
                          {tx.status === 'failed' ? (
                            <XCircle size={20} />
                          ) : tx.type === 'credit' ? (
                        ...
                        <div>
                          <p className="font-bold text-[#292522] text-sm">
                            {tx.description ||
                              (tx.type === 'credit' ? 'Funds Added' : 'Payment Made')}
                          </p>
                          <p className="text-xs text-[#6B5E55]">
                      ...
                      <div className="text-right">
                        <p
                          className={`font-bold text-sm ${tx.status === 'failed' ? 'text-red-500 line-through opacity-70' : tx.type === 'credit' ? 'text-green-700' : 'text-[#292522]'}`}
                        >
                          {tx.type === 'credit' ? '+' : '-'}
                          {formatPrice(tx.amount)}
                        </p>
                        <p
                          className={`text-xs capitalize font-bold ${tx.status === 'failed' ? 'text-red-500' : 'text-[#6B5E55]'}`}
                        >
                          {tx.status}
                        </p>
                      </div>
