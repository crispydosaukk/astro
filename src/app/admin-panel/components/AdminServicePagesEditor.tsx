...
export default function AdminServicePagesEditor() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (selectedService === 'ishta-devata')
        await updateServicePageContent('ishta-devata', ishta);
      else if (selectedService === 'muhurtham')
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          <textarea
            ...
            className="w-full px-4 py-2 rounded-xl bg-background border border-border text-sm resize-none"
          />
        </div>
      </div>
    </>
  ...
  const renderBenefitsEditor = (
    ...
    updateBenefits: (val: string[]) => void
  ) => (
    <>
    ...
    <div className="space-y-6">
      ...
      >
        ...
        {selectedService === 'homa' && (
          <>
            {renderHeroEditor(homa.hero, (f, v) =>
              setHoma({ ...homa, hero: { ...homa.hero, [f]: v } })
            )}
            {renderBenefitsEditor(
              homa.benefitsTitle,
              homa.benefits,
              (v) => setHoma({ ...homa, benefitsTitle: v }),
              (v) => setHoma({ ...homa, benefits: v })
            )}
            <h3 className="text-lg font-bold text-foreground border-b border-border pb-4 mt-8">
              Homam Types
            </h3>
        ...
        {selectedService === 'gemstone' && (
          <>
            {renderHeroEditor(gemstone.hero, (f, v) =>
              setGemstone({ ...gemstone, hero: { ...gemstone.hero, [f]: v } })
            )}
            {renderBenefitsEditor(
              gemstone.benefitsTitle,
              gemstone.benefits,
              (v) => setGemstone({ ...gemstone, benefitsTitle: v }),
              (v) => setGemstone({ ...gemstone, benefits: v })
            )}
            <h3 className="text-lg font-bold text-foreground border-b border-border pb-4 mt-8">
              Gemstones
            </h3>
        ...
        {selectedService === 'mantra' && (
          <>
            {renderHeroEditor(mantra.hero, (f, v) =>
              setMantra({ ...mantra, hero: { ...mantra.hero, [f]: v } })
            )}
            {renderBenefitsEditor(
              mantra.benefitsTitle,
              mantra.benefits,
              (v) => setMantra({ ...mantra, benefitsTitle: v }),
              (v) => setMantra({ ...mantra, benefits: v })
            )}
            <h3 className="text-lg font-bold text-foreground border-b border-border pb-4 mt-8">
              Mantras
            </h3>
            {mantra.mantras.map((m, idx) => (
              <div
                key={idx}
                className="p-4 border border-border rounded-xl bg-muted/20 grid md:grid-cols-2 gap-4"
              >
        ...
        {selectedService === 'yantra' && (
          <>
            {renderHeroEditor(yantra.hero, (f, v) =>
              setYantra({ ...yantra, hero: { ...yantra.hero, [f]: v } })
            )}
            {renderBenefitsEditor(
              yantra.benefitsTitle,
              yantra.benefits,
              (v) => setYantra({ ...yantra, benefitsTitle: v }),
              (v) => setYantra({ ...yantra, benefits: v })
            )}
            <h3 className="text-lg font-bold text-foreground border-b border-border pb-4 mt-8">
              Yantras
            </h3>
        ...
        {selectedService === 'ishta-devata' && (
          <>
            {renderHeroEditor(ishta.hero, (f, v) =>
              setIshta({ ...ishta, hero: { ...ishta.hero, [f]: v } })
            )}
            {renderBenefitsEditor(
              ishta.benefitsTitle,
              ishta.benefits,
              (v) => setIshta({ ...ishta, benefitsTitle: v }),
              (v) => setIshta({ ...ishta, benefits: v })
            )}
            <h3 className="text-lg font-bold text-foreground border-b border-border pb-4 mt-8">
              Deities
            </h3>
        ...
        {selectedService === 'muhurtham' && (
          <>
            {renderHeroEditor(muhurtham.hero, (f, v) =>
              setMuhurtham({ ...muhurtham, hero: { ...muhurtham.hero, [f]: v } })
            )}
            {renderBenefitsEditor(
              muhurtham.benefitsTitle,
              muhurtham.benefits,
              (v) => setMuhurtham({ ...muhurtham, benefitsTitle: v }),
              (v) => setMuhurtham({ ...muhurtham, benefits: v })
            )}
            <h3 className="text-lg font-bold text-foreground border-b border-border pb-4 mt-8">
              Events
            </h3>
            {muhurtham.events.map((ev, idx) => (
              <div
                key={idx}
                className="p-4 border border-border rounded-xl bg-muted/20 grid md:grid-cols-3 gap-4"
              >
        ...
        {selectedService === 'vastu' && (
          <>
            {renderHeroEditor(vastu.hero, (f, v) =>
              setVastu({ ...vastu, hero: { ...vastu.hero, [f]: v } })
            )}
            {renderBenefitsEditor(
              vastu.benefitsTitle,
              vastu.benefits,
              (v) => setVastu({ ...vastu, benefitsTitle: v }),
              (v) => setVastu({ ...vastu, benefits: v })
            )}
            <h3 className="text-lg font-bold text-foreground border-b border-border pb-4 mt-8">
              Directions
            </h3>
        ...
        {selectedService === 'charity' && (
          <>
            {renderHeroEditor(charity.hero, (f, v) =>
              setCharity({ ...charity, hero: { ...charity.hero, [f]: v } })
            )}
            {renderBenefitsEditor(
              charity.benefitsTitle,
              charity.benefits,
              (v) => setCharity({ ...charity, benefitsTitle: v }),
              (v) => setCharity({ ...charity, benefits: v })
            )}
            <h3 className="text-lg font-bold text-foreground border-b border-border pb-4 mt-8">
              Charity Items
            </h3>
