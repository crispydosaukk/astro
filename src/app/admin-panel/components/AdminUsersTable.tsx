    ...
    <div className="glass-card-light dark:glass-card rounded-2xl border border-border overflow-hidden relative min-h-[400px]">
      ...
      <div className="p-5 border-b border-border flex flex-wrap items-center gap-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 rounded-xl bg-muted border border-border text-sm outline-none focus:border-ring transition-all w-48"
          />
        </div>
        <select
          value={filterPlan}
          ...
          <tbody>
            {paginated?.map((user, i) => (
              ...
              >
                ...
                <td className="px-5 py-4">
                  <span
                    className={`px-2.5 py-1 rounded-full text-xs font-semibold ${user?.plan === 'Annual' ? 'bg-purple-500/15 text-purple-400' : user?.plan === 'Premium' ? 'bg-accent/15 text-accent' : 'bg-muted text-muted-foreground'}`}
                  >
                    {user?.plan}
                  </span>
                </td>
                ...
                <td className="px-5 py-4 text-foreground font-mono tabular-nums">
                  {user?.reports}
                </td>
                <td className="px-5 py-4 text-foreground font-mono tabular-nums">
                  {user?.consultations}
                ...
                <td className="px-5 py-4">
                  <span
                    className={`px-2.5 py-1 rounded-full text-xs font-semibold ${user?.status === 'active' ? 'bg-green-500/15 text-green-400' : 'bg-red-500/15 text-red-400'}`}
                  >
      ...
      <div className="px-5 py-4 border-t border-border flex items-center justify-between text-sm">
        <span className="text-muted-foreground">
          Showing {Math.min(filtered?.length, perPage)} of {filtered?.length} users
        </span>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setPage((p) => Math.max(1, p - 1))}
            disabled={page === 1}
            className="p-2 rounded-lg hover:bg-muted transition-all disabled:opacity-40"
          >
            <ChevronLeft size={14} />
          </button>
          {Array.from({ length: totalPages }, (_, i) => i + 1)?.map((p) => (
            <button
              key={`page-${p}`}
              onClick={() => setPage(p)}
              className={`w-8 h-8 rounded-lg text-xs font-medium transition-all ${page === p ? 'gold-gradient-bg text-white' : 'hover:bg-muted text-muted-foreground'}`}
            >
              {p}
            </button>
          ))}
