...
'4:30 PM', '5:00 PM', '6:00 PM', '6:30 PM', '7:00 PM', '7:30 PM'];
...
export default function ConsultationContent() {
  ...
  const handleBook = async () => {
    ...
    setBookingStep(1);
    toast.success(`Consultation booked with ${selectedAstrologer?.name}! You'll receive a confirmation on WhatsApp.`);
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-30 bg-card/80 backdrop-blur-md border-b border-border px-6 lg:px-8 py-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">Connect with 500+ verified Vedic astrologers</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-[#C9952B]/10 border border-[#C9952B]/30">
              ...
              <select value={selectedCurrency.code} onChange={(e) => setSelectedCurrency(currencies.find(c => c.code === e.target.value) || currencies[0])} className="bg-transparent text-xs font-semibold text-[#C9952B] outline-none cursor-pointer">
      ...
      <div className="px-6 lg:px-8 py-6 max-w-screen-2xl">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-4 py-2.5 rounded-xl bg-muted border border-border text-sm outline-none focus:border-ring transition-all">
            <option value="all">All Status</option>
            <option value="online">Online Now</option>
            <option value="busy">Busy</option>
          </select>

          <select value={filterSpecialty} onChange={(e) => setFilterSpecialty(e.target.value)} className="px-4 py-2.5 rounded-xl bg-muted border border-border text-sm outline-none focus:border-ring transition-all">
            {specialties.map((s) =>
            <option key={`spec-filter-${s}`} value={s}>{s === 'all' ? 'All Specialties' : s}</option>
            )}
          </select>

          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="px-4 py-2.5 rounded-xl bg-muted border border-border text-sm outline-none focus:border-ring transition-all">
          ...
          <div className="text-sm text-muted-foreground ml-auto">{filtered.length} astrologers found</div>
        </div>
            ...
            className="glass-card-light dark:glass-card rounded-2xl border border-border overflow-hidden card-hover group">
              ...
              <div className="p-5">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-1">
                      ...
                      {ast.badge &&
                    <span className="text-xs px-1.5 py-0.5 rounded-full bg-accent/15 text-accent font-semibold flex-shrink-0">{ast.badge}</span>
                    }
                    </div>
                ...
                <div className="flex flex-wrap gap-1 mb-3">
                  {ast.specialty.slice(0, 2).map((s) =>
                <span key={`spec-${ast.id}-${s}`} className="text-xs px-2 py-0.5 rounded-full bg-secondary/10 text-secondary font-medium">{s}</span>
                )}
                  {ast.specialty.length > 2 &&
                <span className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">+{ast.specialty.length - 2}</span>
                }
                </div>

                <div className="flex items-center justify-between mb-4">
                  <div>
                    <span className="text-lg font-bold text-accent tabular-nums">{selectedCurrency.symbol}{convertPrice(ast.pricePerMin)}</span>
                    <span className="text-xs text-muted-foreground">/min</span>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {ast.languages.slice(0, 2).map((l) =>
                  <span key={`lang-card-${ast.id}-${l}`} className="text-xs px-2 py-0.5 rounded-full bg-muted text-muted-foreground">{l}</span>
                  )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                  onClick={() => {setSelectedAstrologer(ast);setConsultationType('video');setBookingStep(1);}}
                  disabled={ast.status === 'offline'}
                  className="flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold bg-secondary/10 text-secondary hover:bg-secondary hover:text-white transition-all disabled:opacity-40 disabled:cursor-not-allowed">
                  
                    <Video size={12} /> Video
                  </button>
                  <button
                  onClick={() => {setSelectedAstrologer(ast);setConsultationType('call');setBookingStep(1);}}
                  disabled={ast.status === 'offline'}
      ...
      <AnimatePresence>
        ...
        <motion.div
          ...
          onClick={(e) => {if (e.target === e.currentTarget) {setSelectedAstrologer(null);setBookingStep(1);}}}>
            ...
            className="bg-card rounded-2xl border border-border w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <React.Fragment key={`booking-step-${s.n}`}>
                    ...
                    {si < 2 && <div className={`flex-1 h-0.5 mx-2 transition-all ${bookingStep > s.n ? 'bg-accent' : 'bg-border'}`} />}
                  </React.Fragment>
              )}
              </div>
              ...
              <div className="space-y-5">
                    ...
                    <button
                      ...
                      className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 ${consultationType === type ? 'border-accent bg-accent/10' : 'border-border hover:border-accent/50'}`}>
                            ...
                            {type === 'video' ? <Video size={24} className={consultationType === type ? 'text-accent' : 'text-muted-foreground'} /> : <Phone size={24} className={consultationType === type ? 'text-accent' : 'text-muted-foreground'} />}
                            <span className="font-semibold text-sm text-foreground capitalize">{type} Consultation</span>
                            <span className="text-xs text-muted-foreground">{type === 'video' ? 'Face-to-face via video' : 'Audio only'}</span>
                          </button>
                    )}
                      </div>
                    </div>
              ...
              <div className="space-y-5">
                    ...
                    <div key={`day-header-${d}`} className="text-center text-xs font-medium text-muted-foreground py-1">{d}</div>
                    )}
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setBookingStep(1)} className="flex-1 py-3 rounded-xl border border-border text-sm font-semibold hover:border-accent/50 transition-all">
                        Back
                      </button>
                      <button
              ...
              <div className="space-y-5">
                  ...
                  <div key={`summary-${item.label}`} className="flex justify-between text-sm">
                      ...
                      <div className="border-t border-border pt-3 flex justify-between">
                        ...
                        <span className="text-xl font-bold text-accent tabular-nums">{selectedCurrency.symbol}{totalCostConverted}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 p-3 rounded-xl bg-accent/10 border border-accent/20">
                      ...
                      <p className="text-xs text-muted-foreground">Paying in <span className="font-semibold text-accent">{selectedCurrency.label} ({selectedCurrency.code})</span> · Exchange rates are approximate</p>
                    </div>
                    ...
                    map((method) =>
                    <button key={method.id} className="flex items-center gap-2 p-3 rounded-xl border border-border hover:border-accent/50 text-sm font-medium hover:text-accent transition-all">
                    ...
                    <div className="flex gap-3">
                      <button onClick={() => setBookingStep(2)} className="flex-1 py-3 rounded-xl border border-border text-sm font-semibold hover:border-accent/50 transition-all">
                        Back
                      </button>
                      <button
                    onClick={handleBook}
                    ...
                    <><CreditCard size={16} /> Pay {selectedCurrency.symbol}{totalCostConverted}</>
                    }
                      </button>
                    </div>
                  </div>
