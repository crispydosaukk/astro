...
import { FileText, Download, Eye, Gem, Music, Triangle, Flame, ArrowRight, X, Loader2 } from 'lucide-react';
...
export default function RecentReports() {
  ...
  return (
    <>
      <div id="recent-reports" className="glass-card-light dark:glass-card rounded-2xl border border-border overflow-hidden">
        <div className="p-6 border-b border-border flex items-center justify-between">
          <h2 className="text-lg font-bold text-foreground">Recent Reports</h2>
          <Link href="/#services" className="flex items-center gap-1 text-xs text-accent hover:underline">
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-9 h-9 rounded-xl ${getBgClassForType(rep.type)} flex items-center justify-center flex-shrink-0`}>
                            <Icon size={15} className={getColorClassForType(rep.type)} />
                          </div>
                          <span className="font-medium text-foreground">{rep.type || 'Custom Report'}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-muted-foreground font-mono text-xs">{formatDate(rep.createdAt)}</td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2 transition-opacity">
                          <button onClick={() => setSelectedReport(rep)} className="p-1.5 rounded-lg hover:bg-muted transition-all" title="View report">
              ...
              <div className="p-6 border-b border-border flex items-center justify-between bg-muted/30">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Generated for {selectedReport.details?.dob} | {selectedReport.details?.time} | {selectedReport.details?.place}
                  </p>
                </div>
                <button
                  onClick={() => setSelectedReport(null)}
                        ...
                        <div className="space-y-4">
                          <div className="p-5 rounded-2xl bg-[#C9952B]/10 border border-[#C9952B]/30">
                            <p className="text-xs text-[#C9952B] font-semibold mb-1 uppercase tracking-wider">
                              {data.recommendationTitle.replace(/Recommended /i, '').replace(/Get Your /i, '')}
                            </p>
                            <h2 className="text-2xl font-bold text-foreground mb-2">{data.recommendationName}</h2>
                            <p className="text-sm text-muted-foreground font-medium flex items-center gap-2">
                               {data.timing} {data.duration && data.duration !== 'N/A' && `· ${data.duration}`}
                            </p>
                          </div>
                          
                          {data.materials && (
                            <div className="p-5 rounded-2xl bg-card border border-border shadow-sm">
                              <p className="text-xs text-muted-foreground font-semibold mb-2 uppercase tracking-wider">Materials / Requirements</p>
                              <p className="text-sm text-foreground leading-relaxed">{data.materials}</p>
                            </div>
                          )}
                          
                          {(data.astrologicalAnalysis || data.description) && (
                            <div className="p-5 rounded-2xl bg-card border border-border shadow-sm">
                              <p className="text-xs text-muted-foreground font-semibold mb-2 uppercase tracking-wider">Astrological Insights</p>
                          ...
                          {data.procedure && (
                            <div className="p-5 rounded-2xl bg-card border border-border shadow-sm">
                              <p className="text-xs text-muted-foreground font-semibold mb-2 uppercase tracking-wider">Procedure / Methodology</p>
                          ...
                          {data.rules && (
                            <div className="p-5 rounded-2xl bg-card border border-[#C9952B]/20 shadow-sm relative overflow-hidden">
                              ...
                              <p className="text-xs text-[#C9952B] font-semibold mb-2 uppercase tracking-wider pl-2">Rules & Restrictions</p>
                              <p className="text-sm text-foreground leading-relaxed pl-2">{data.rules}</p>
                            </div>
                          )}
                        </div>
                      );
                  ...
                  return (
                    <div className="prose prose-invert prose-p:text-muted-foreground prose-headings:text-foreground prose-strong:text-foreground max-w-none">
                      {selectedReport.reportContent?.split('\n').map((line: string, i: number) => {
                        if (line.startsWith('### ')) return <h4 key={i} className="text-lg font-bold mt-6 mb-3 text-[#C9952B]">{line.replace('### ', '')}</h4>;
                        if (line.startsWith('## ')) return <h3 key={i} className="text-xl font-bold mt-8 mb-4 text-[#C9952B]">{line.replace('## ', '')}</h3>;
                        if (line.startsWith('# ')) return <h2 key={i} className="text-2xl font-bold mt-8 mb-4 text-[#C9952B]">{line.replace('# ', '')}</h2>;
                        if (line.startsWith('- ')) return <li key={i} className="ml-4 mb-1">{line.substring(2)}</li>;
                        if (line.trim() === '') return <br key={i} />;
                        return <p key={i} className="mb-4 leading-relaxed">{line.replace(/\*\*(.*?)\*\*/g, '$1')}</p>;
                      })}
                    </div>
                  );
                })()}
