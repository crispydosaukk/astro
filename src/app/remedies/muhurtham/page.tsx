...
import { Compass, Check, ArrowRight, Lock, Loader2, Bot } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background dark text-foreground">
      ...
      <section className="relative overflow-hidden border-b border-[#B88A44]/20 flex flex-col justify-center min-h-[85vh] lg:min-h-[90vh] pt-24 lg:pt-28 pb-16 lg:pb-24">
        ...
        <div className="absolute inset-0 z-0 select-none pointer-events-none">
          <Image
            src="/assets/images/remedies/remedies_muhurtham_1785738473891.png"
            alt="Muhurtham Generator Background"
            fill
            ...
            <div className="max-w-3xl space-y-6">
              ...
              >
                <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs sm:text-sm font-bold tracking-wide bg-[#B88A44]/20 text-[#F6D075] border border-[#B88A44]/40 shadow-xl shadow-black/20 backdrop-blur-md">
                  ...
                  {content.hero.tag || 'Auspicious Vedic Timings'}
                </span>
              </motion.div>
              ...
              >
                ...
                >
                  <Compass size={18} />
                  <span>{content.hero.primaryBtnText || 'Find Auspicious Muhurtham'}</span>
                </a>
                <Link
        ...
        <div className="max-w-screen-2xl mx-auto px-6 lg:px-10 space-y-8">
          <div className="text-center space-y-2">
            ...
            <h2 className="text-2xl sm:text-3xl font-bold text-[#292522]">
              {content.eventsTitle || 'Muhurtham Calculation for Key Life Events'}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
            {content.events?.map((e, i) => (
              <motion.div
                ...
                className="rounded-3xl border border-[#E5D9C8] bg-[#F8F3EA] p-6 text-center space-y-2 shadow-sm hover:shadow-md hover:border-[#B88A44] transition-all"
              >
                <div className="text-3xl mb-1">{e?.icon}</div>
                <h3 className="font-bold text-[#292522] text-base">{e?.name}</h3>
                <p className="text-xs text-[#6B5E55] leading-relaxed">{e?.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
            ...
            <div className="bg-[#FFFDFC] p-8 sm:p-10 rounded-3xl border border-[#E5D9C8] space-y-6 shadow-xl flex flex-col justify-between">
              <div>
                ...
                <h2 className="text-2xl sm:text-3xl font-bold text-[#292522] mb-6">
                  {content.benefitsTitle || 'Why Beginning at the Right Time Ensures Success'}
                </h2>
