...
export interface PanchangData {
  ...
  nightChoghadiya: Array<{ num: number; name: string; type: 'Good' | 'Neutral' | 'Bad' | 'Evil'; start: string; end: string; isCurrent?: boolean }>;
  planetaryPositions: Array<{ planet: string; rashi: string; lon: string; nakshatra: string; pada: number }>;
  coordinates: { lat: number; lon: number };
}

export const CITY_COORDINATES: Record<string, { lat: number; lon: number; tzOffsetHours?: number }> = {
  ...
  'new york': { lat: 40.7128, lon: -74.0060, tzOffsetHours: -4 },
...
export function getCoordinatesForLocation(locationName: string): { lat: number; lon: number; tzOffset: number } {
  if (!locationName) {
    return { lat: 17.3850, lon: 78.4867, tzOffset: 5.5 }; // Hyderabad default
  }
...
export function calculatePanchang(dateInput?: string, locationName: string = 'Hyderabad, Telangana, India'): PanchangData {
  const targetDate = dateInput ? new Date(dateInput + (dateInput.includes('T') ? '' : 'T00:00:00')) : new Date();
  ...
  const shubhMuhurats = [
    { name: 'Abhijit Muhurat', desc: 'Most powerful muhurat for all auspicious tasks', timing: `${abhijitStartStr} – ${abhijitEndStr}`, isBest: true },
    { name: 'Brahma Muhurat', desc: 'Spiritual practice, yoga & meditation', timing: `${formatTime24to12(brahmaStartMins)} – ${formatTime24to12(brahmaEndMins)}` },
    { name: 'Amrit Kaal', desc: 'Favourable for new beginnings & ventures', timing: `${formatTime24to12(amritKaalStartMins)} – ${formatTime24to12(amritKaalEndMins)}` },
    { name: 'Vijay Muhurat', desc: 'Success in lawsuits, challenges & competitions', timing: `${formatTime24to12(vijayStartMins)} – ${formatTime24to12(vijayEndMins)}` },
    { name: 'Godhuli Muhurat', desc: 'Twilight evening prayer & peace', timing: `${formatTime24to12(godhuliStartMins)} – ${formatTime24to12(godhuliEndMins)}` },
    { name: 'Nishita Muhurat', desc: 'Midnight spiritual dhyana & sadhana', timing: `${formatTime24to12(nishitaStartMins)} – ${formatTime24to12(nishitaEndMins)}` },
    { name: 'Pratah Sandhya', desc: 'Morning twilight sacred ritual window', timing: `${formatTime24to12(pratahSandhyaStartMins)} – ${formatTime24to12(pratahSandhyaEndMins)}` },
    { name: 'Sayahna Sandhya', desc: 'Evening sunset sacred ritual window', timing: `${formatTime24to12(sayahnaSandhyaStartMins)} – ${formatTime24to12(sayahnaSandhyaEndMins)}` },
  ];
  ...
  const planetaryPositions = [
    { planet: 'Ascendant', rashi: 'Cancer', lon: `${(25 + (dayIndex % 5) + Math.round(lon % 5)) % 30}°0′17″`, nakshatra: 'Ashlesha', pada: 4 },
    { planet: 'SUN', rashi: 'Cancer', lon: `${(28 + (dayIndex % 2)) % 30}°54′52″`, nakshatra: 'Ashlesha', pada: 4 },
    { planet: 'MOON', rashi: 'Virgo', lon: `${(15 + (dayIndex % 14) + Math.round(lon % 3)) % 30}°24′33″`, nakshatra: 'Chitra', pada: 1 },
