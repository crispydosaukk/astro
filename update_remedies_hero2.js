...
const remedies = ['charity', 'fasting', 'homa', 'ishta-devata', 'muhurtham', 'rudraksha', 'vastu', 'yantra']; // skip gemstone and mantra since they are done
...
const imageMap = {
  ...
  'fasting': '/assets/images/remedies/remedies_homam_1785738443734.png',
};
...
for (const remedy of remedies) {
  ...
  // Find the icon used in the hero
  const tagIconMatch = content.match(/<([A-Za-z0-9]+)\s+size=\{12\}/);
  const tagIcon = tagIconMatch ? tagIconMatch[1] : 'Star';
  ...
  const oldHeroRegex = /<section className="relative pt-24 py-20 cosmic-bg overflow-hidden">[\s\S]*?<\/section>/;
  if (oldHeroRegex.test(content)) {
    // Optionally remove '{/* Hero */}' if it exists before the section
    content = content.replace(/\{\/\*\s*Hero\s*\*\/\}\s*<section className="relative pt-24 py-20 cosmic-bg overflow-hidden">[\s\S]*?<\/section>/, newHero);
    // If not removed above, do normal replace
